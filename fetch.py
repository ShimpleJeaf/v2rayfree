...
class Node:
    ...
    def __hash__(self):
        ...
        try:
            path = ""
            if self.type == 'vmess' or self.type == 'socks' or self.type == 'vless' or self.type == 'hysteria2':
                path = data['network']+':'
                if data['network'] == 'ws':
                    if 'ws-opts' in data:
                        try:
                ...
                if data['network'] == 'ws':
                    if 'ws-opts' in data:
                        ...
                        if 'path' in data['ws-opts']:
                            path += '/'+data['ws-opts']['path']
            complete_hashstr = f"{self.type}:{data['server']}:{data['port']}:{data['uuid']}:{path}"
            simple_hashstr = f"{self.type}:{data['server']}:{data['port']}"
            complete_hash = hash(complete_hashstr)
            if complete_hash in usedhash:
                return hash('__ERROR__')
            else:
                usedhash.add(complete_hash)
            return hash(simple_hashstr)
    ...
    def load_url(self, url: str) -> None:
        ...
        if self.type == 'vmess':
            ...
            if has_para:
                if '#' in d:
                    parastr, beizhu = d.split('#')
                else:
                    parastr = d
            elif has_beizhu:
                beizhu = d
            parastrs = parastr.split('&')
            paras = {}
            for parastr in parastrs:
                if not parastr: continue
                key, value = parastr.split('=')
                paras[key] = value
            self.data['name'] = uuid
            self.data['server'] = add
            self.data['port'] = port
            self.data['uuid'] = uuid
            self.data['network'] = ''
            self.data['tls'] = ''
            self.data['servername'] = ''

            if 'type' in paras:
                self.data['network'] = paras['type']
            if 'security' in paras:
                self.data['tls'] = paras['security'] == 'tls'
            if 'sni' in paras:
                self.data['servername'] = paras['sni']
...
same: Dict[int, List[Node]] = {} # ip和port相同的节点
...
def merge(source_obj: Source, sourceId=-1) -> None:
    ...
    for p in sub:
        ...
        try: n = Node(p)
            ...
            #if len(e.args) == 1:
            print(f"不支持的类型：{e}")
            logger.error(f"不支持的类型：{e}")
            logger.error(f"url: {p}")
            unknown.add(p)
        except: 
            traceback.print_exc()
            print("在解析 '"+p+"' 时发生错误：\n")
...
def main():
    ...
    print(f"共有 {sames + len(merged)-unsupports} 个正常节点，{len(unknown)} 个无法解析的节点，共",
            sames + len(merged)+len(unknown),f"个。{unsupports} 个节点不被 V2Ray 支持。")
